...
public class MotorVelocitySystem {
    ...
    public MotorVelocitySystem(double kS, double kV, double kA, double error) {
        ...
        observer = new KalmanFilter<>(Nat.N1(), Nat.N1(), plant, MODEL_STD_DEV, ENCODER_STD_DEV, Constants.ROBOT_LOOP);
        controller = new LinearQuadraticRegulator<>(plant, VecBuilder.fill(error), QELMS, Constants.ROBOT_LOOP);
